...
export class RoadSystem {
  ...
  private createRoadModels(): void {
    ...
    // Base da rua (asfalto)
    const roadBaseGeometry = new THREE.BoxGeometry(
      this.tileSize,
      0.1,
    ...
    const intersectionBase = new THREE.Mesh(
      intersectionBaseGeometry,
      roadMaterial,
    );
    ...
    for (let i = -1; i <= 1; i += 2) {
      ...
      line.position.set(i * (this.laneWidth + 0.1), 0.01, -this.tileSize / 2);
    ...
    for (let i = -1; i <= 1; i += 2) {
      const line = new THREE.Mesh(intersectionLineGeometry, lineMaterial);
      line.position.set(this.tileSize / 2, 0.01, i * (this.laneWidth + 0.1));
      intersectionModel.add(line);
    }
  ...
  public setTile(
    ...
    y: number,
    type: RoadTileType,
    orientation: RoadOrientation = RoadOrientation.HORIZONTAL,
  ): void {
    if (
      y >= 0 &&
      y < this.roadMap.length &&
      x >= 0 &&
      x < this.roadMap[y].length
    ) {
  ...
  public setRoadMap(mapData: number[][]): void {
    ...
    for (let y = 0; y < height; y++) {
      for (let x = 0; x < width; x++) {
        ...
        const orientation =
          value & 0b100 ? RoadOrientation.VERTICAL : RoadOrientation.HORIZONTAL;
    ...
    for (let y = 0; y < height; y++) {
      for (let x = 0; x < width; x++) {
        ...
        if (tile.type === RoadTileType.STRAIGHT) {
          ...
          if (
            (hasIntersectionNorth && hasIntersectionSouth) ||
          ...
          // 3. Se não tiver interseções, verificar ruas retas
            ...
            (hasStraightNorth && hasStraightSouth) ||
            (hasStraightNorth &&
              tile.orientation === RoadOrientation.VERTICAL) ||
            ...
            (hasStraightEast && hasStraightWest) ||
            (hasStraightEast &&
              tile.orientation === RoadOrientation.HORIZONTAL) ||
  ...
  private getStraightRoadNeighbors(
    x: number,
    y: number,
