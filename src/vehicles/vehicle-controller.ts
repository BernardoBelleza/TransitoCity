...
export class VehicleController {
  ...
  public update(deltaTime: number): void {
    ...
    // Avança o veículo
    this.tileProgress +=
      (this.speed * deltaTime) / this.roadSystem.getTileSize();
  ...
  private moveToNextTile(): void {
    ...
    const nextPosition = this.roadSystem.getTileWorldPosition(
      this.currentTileX,
      this.currentTileY,
    );
    if (!nextPosition) {
    ...
    const nextTile = this.roadSystem.getTileInfo(
      this.currentTileX,
      this.currentTileY,
    );
  ...
  private decideDirectionAtIntersection(intersectionTile: RoadTile): void {
    ...
    for (let dir = 0; dir < 4; dir++) {
      if (dir !== oppositeDirection) {
        ...
        const nextX =
          this.currentTileX +
          this.getOffsetXForDirection(dir as VehicleDirection);
        const nextY =
          this.currentTileY +
          this.getOffsetYForDirection(dir as VehicleDirection);
        ...
        if (nextTile && nextTile.type !== RoadTileType.EMPTY) {
          ...
          if (nextTile.type === RoadTileType.STRAIGHT) {
            const isCompatible = this.isDirectionCompatibleWithRoad(
              dir as VehicleDirection,
              nextTile,
            );
            if (isCompatible) {
  ...
  private isDirectionCompatibleWithRoad(
  ...
  private adjustDirectionForStraightRoad(roadTile: RoadTile): void {
    ...
    if (roadTile.orientation === RoadOrientation.VERTICAL) {
      ...
      if (
        ...
        const northTile = this.roadSystem.getTileInfo(
          this.currentTileX,
          this.currentTileY - 1,
        );
        const southTile = this.roadSystem.getTileInfo(
          this.currentTileX,
          this.currentTileY + 1,
        );
    ...
    // Para ruas horizontais, a direção deve ser Leste ou Oeste
      ...
      if (
        ...
        const eastTile = this.roadSystem.getTileInfo(
          this.currentTileX + 1,
          this.currentTileY,
        );
        const westTile = this.roadSystem.getTileInfo(
          this.currentTileX - 1,
          this.currentTileY,
        );
  ...
  private getOppositeDirection(): VehicleDirection {
    switch (this.currentDirection) {
      case VehicleDirection.NORTH:
        return VehicleDirection.SOUTH;
      case VehicleDirection.SOUTH:
        return VehicleDirection.NORTH;
      case VehicleDirection.EAST:
        return VehicleDirection.WEST;
      case VehicleDirection.WEST:
        return VehicleDirection.EAST;
    }
  }

  // Verifica se é uma curva à direita
  private isRightTurn(
    fromDirection: VehicleDirection,
    toDirection: VehicleDirection,
  ): boolean {
    return (
      (fromDirection === VehicleDirection.NORTH &&
        toDirection === VehicleDirection.EAST) ||
      (fromDirection === VehicleDirection.EAST &&
        toDirection === VehicleDirection.SOUTH) ||
      (fromDirection === VehicleDirection.SOUTH &&
        toDirection === VehicleDirection.WEST) ||
      (fromDirection === VehicleDirection.WEST &&
        toDirection === VehicleDirection.NORTH)
    );
  }
